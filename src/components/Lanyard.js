...
export default function Lanyard({ position = [0, 0, 30], gravity = [0, -40, 0], fov = 15, transparent = true, cardImage = '/profile.jpg' }) {
    ...
    return (
        <div className="lanyard-wrapper">
            <Canvas
                camera={{ position: position, fov: isMobile ? fov * 1.8 : fov }}
...
function Band({ maxSpeed = 50, minSpeed = 0, isMobile = false, cardImage }) {
    ...
    // Load the user's profile image for the card
    const rawCardTexture = useTexture(cardImage || '/profile.jpg');
    const cardTexture = useMemo(() => {
        const t = rawCardTexture.clone();
        t.wrapS = THREE.RepeatWrapping;
        t.wrapT = THREE.RepeatWrapping;
        t.flipY = false;

        // Card dimensions from collider: width 1.6, height 2.25
        const cardWidth = 1.6;
